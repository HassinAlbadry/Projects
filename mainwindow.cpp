    ...
    : QMainWindow(parent), mPrevInstrumentSetted(false)
{
	...
	initializeZoomSlider(); // new function
    initializeStatusBar();
    initializeTabWidget();
...
void MainWindow::initializeMainMenu()
{
	...
	QAction *mDashlineAction = new QAction(tr("Dashline"), this);
	mDashlineAction->setCheckable(true);
	mDashlineAction->setIcon(QIcon(":/media/instruments-icons/dashline.png"));
	connect(mDashlineAction, SIGNAL(triggered(bool)), this, SLOT(instumentsAct(bool)));
	mInstrumentsMenu->addAction(mDashlineAction);
	mInstrumentsActMap.insert(DASHLINE, mDashlineAction);
...
{
	mPaletteBar->addSeparator();

	QSplitter *testSplitter = new QSplitter();
	QLabel *label = new QLabel();
	label->setText("Zoom:");
	label->setAlignment(Qt::AlignRight);
	testSplitter->addWidget(label);

	QSlider *zoomSlider = new QSlider(Qt::Horizontal);
	zoomSlider->setRange(25, 400);
	zoomSlider->setValue(100);
	connect(zoomSlider, SIGNAL(valueChanged(int)), this, SLOT(zoomSliderAct(int)));
	connect(this, SIGNAL(sendNewZoomFactor(int)), zoomSlider, SLOT(setValue(int)));

	testSplitter->addWidget(zoomSlider);
	QList<int> sizeList;
	sizeList << 700 << 300;
	testSplitter->setSizes(sizeList);
	testSplitter->setCollapsible(0, false);
	testSplitter->setCollapsible(1, false);

	mPaletteBar->addWidget(testSplitter);
}
...
void MainWindow::updateShortcuts()
{
	...
	mInstrumentsActMap[DASHLINE]->setShortcut(DataSingleton::Instance()->getInstrumentShortcutByKey("Dashline"));
...
void MainWindow::zoomInAct()
{
	...
	// Signal to zoomSlider
	emit sendNewZoomFactor(getCurrentImageArea()->getZoomFactor() * 100);
}
...
void MainWindow::zoomSliderAct(int delta)
{
	qreal factor = (delta / 100.0) / getCurrentImageArea()->getZoomFactor();
